...
Package `com.veszelovszki.signboard`. Kotlin, no dependencies, one source file.

## Layout

- `src/main/kotlin/com/veszelovszki/signboard/MainActivity.kt`: the whole app. Views are built in
  code; there are no layout XML files.
...
The release APK is ~27 KB. It got there by having no dependencies at all, and `scripts/check.sh`
...
2. `./scripts/check.sh`
3. `gradle bundleRelease` → `build/outputs/bundle/release/Signboard-release.aab`, which is what Play takes.
